...
import { useCreateUserRagIndexMutation } from '../Rag/api'
import { RagCreatorDialog, RagCreatorDialogSubmitPayload } from '../Rag/RagCreator'
...
const RagSettingsSection = () => {
  ...
  const { t } = useTranslation()
  const { user } = useCurrentUser()
  const { setIsEditing } = usePromptEditorState()
  const [open, setOpen] = useState(false)
  const createUserIndexMutation = useCreateUserRagIndexMutation()
  const [createdIndex, setCreatedIndex] = useState<{ id: number; metadata: { name: string } } | null>(null)
  const [created, setCreated] = useState(false)

  console.log('RagSettingsSection render, created:', created)

  useEffect(() => {
    console.log('RagSettingsSection mounted')

    return () => {
      console.log('RagSettingsSection unmounted')
    }
  }, [])

  useEffect(() => {
    console.log('created effect ran, value:', created)

    if (!created) return

    console.log('created is now true')
  }, [created])
  ...
  const handleDialogSubmit = async ({ name, language }: RagCreatorDialogSubmitPayload) => {
    ...
    const newIndex = await createUserIndexMutation.mutateAsync({ name, language })

    console.log('mutation finished', newIndex)

    setCreated(true)

    console.log('setCreated called')

    setCreatedIndex(newIndex)

    setForm((prev) => ({
      ...prev,
      ragIndexId: newIndex.id,
    }))

    setIsEditing(true)
  }

  console.log('RagSettingsSection render, createdIndex:', createdIndex)
      ...
      <Box mb={3}>
        ...
        <Box display="flex" justifyContent="space-around" alignItems="center" flexDirection="column" gap={2}>
          ...
          <TextButton
            ...
            data-testid="add-source-material-button"
...
export const PromptEditorFormV2 = () => (
  <Box>
    <BasicInfoSection />
    <Divider sx={{ my: 3 }} />
