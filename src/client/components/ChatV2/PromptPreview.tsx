    ...
    <>
      <Paper variant="outlined" sx={{ p: 3, borderRadius: '12px', overflow: 'auto', maxHeight: '100%' }}>
        ...
        {(!isPersonalPrompt || user.isEmployee || user.isAdmin) && (
          <>
            ...
            {ragDetails ? (
              <Paper
                sx={{ p: 2, mt: 1.5, backgroundColor: alpha(theme.palette.primary.main, 0.08), ...(!isMobile && { maxHeight: '300px', overflow: 'auto' }) }}
              >
                <Box
                  sx={{
                    flex: 1,
      ...
      <Dialog open={embeddingModalOpen} onClose={() => setEmbeddingModalOpen(false)} fullWidth maxWidth="lg">
        ...
        <DialogContent dividers>
          <CourseEmbedding courseId={courseId} prompt={prompt} />
        </DialogContent>
      </Dialog>
    </>
  )
